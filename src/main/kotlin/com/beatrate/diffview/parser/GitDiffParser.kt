...
class GitDiffParser : DiffParser {
    ...
    override fun parse(file: File): Commit {
    ...
    private fun parseHunk(reader: PeekReader): Hunk {
        ...
        while (reader.isNotEmpty) {
            ...
            val line = when {
                content.startsWith(Prefix.DELETED_LINE) -> Line(
                    LineKind.DELETED,
                    content.drop(Prefix.DELETED_LINE.length)
                )
                content.startsWith(Prefix.ADDED_LINE) -> Line(LineKind.ADDED, content.drop(Prefix.ADDED_LINE.length))
                content.startsWith(Prefix.REGULAR_LINE) -> Line(
                    LineKind.REGULAR,
                    content.drop(Prefix.REGULAR_LINE.length)
                )
